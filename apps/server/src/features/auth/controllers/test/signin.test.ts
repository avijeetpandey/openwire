...
describe('SignIn', () => {
  ...
  it('should throw an error if username is not available', () => {
    ...
    SignIn.prototype.read(req, res).catch((error: CustomError) => {
      ...
      expect(error.serializErrors().message).toEqual('Username is a required field');
    });
  });
  ...
  it('should throw an error if username length is less than minimum length', () => {
    ...
    SignIn.prototype.read(req, res).catch((error: CustomError) => {
      expect(error.statusCode).toEqual(400);
      expect(error.serializErrors().message).toEqual('Invalid username');
    });
  });
  ...
  it('should throw an error if password is not available', () => {
    ...
    SignIn.prototype.read(req, res).catch((error: CustomError) => {
      expect(error.statusCode).toEqual(400);
      expect(error.serializErrors().message).toEqual('Password is a required field');
    });
  });
  ...
  it('should throw an error if password length is less than minimum length', () => {
    ...
    SignIn.prototype.read(req, res).catch((error: CustomError) => {
      expect(error.statusCode).toEqual(400);
      expect(error.serializErrors().message).toEqual('Invalid password');
    });
  });
  ...
  it('should throw "Invalid credentials" if username does not exist', () => {
    ...
    SignIn.prototype.read(req, res).catch((error: CustomError) => {
      ...
      expect(error.statusCode).toEqual(400);
      expect(error.serializErrors().message).toEqual('Invalid credentials');
    });
  });
