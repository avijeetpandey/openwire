...
import { Create } from '@root/features/posts/controllers/create-post';
  ...
  describe('postWithImage', () => {
    it('should throw an error if image is not available', () => {
      ...
      Create.prototype.postWithImage(req, res).catch((error: CustomError) => {
        ...
        expect(error.serializErrors().message).toEqual('Image is a required field');
      });
    });
    ...
    it('should throw an upload error', () => {
      ...
      Create.prototype.postWithImage(req, res).catch((error: CustomError) => {
        expect(error.statusCode).toEqual(400);
        expect(error.serializErrors().message).toEqual('Upload error');
      });
    });
